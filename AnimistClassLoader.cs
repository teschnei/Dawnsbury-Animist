...
public static class AnimistClassLoader
{
    ...
    private static Trait TAnimist = ModManager.RegisterTrait("Animist", new TraitProperties("Animist", true)
    {
        IsClassTrait = true
    });
    ...
    private static IEnumerable<Feat> CreateFeats()
    {
        ...
        yield return new ClassSelectionFeat(ModManager.RegisterFeatName("FeatAnimist", "Animist"),
...
{b}Level 8:{/b} Portalist feat",
    new List<Feat>() {
                    new Feat(ModManager.RegisterFeatName("Liturgist"), "Giga test from hell", "f", [TAnimist], null),
                    new Feat(ModManager.RegisterFeatName("Medium"), "Giga test from hell", "f", [TAnimist], null),
                    new Feat(ModManager.RegisterFeatName("Seer"), "Giga test from hell", "f", [TAnimist], null),
                    new Feat(ModManager.RegisterFeatName("Shaman"), "Giga test from hell", "f", [TAnimist], null),
                }
            )
            .WithOnSheet(sheet =>
            {
                sheet.AddSelectionOption(new SingleFeatSelectionOption("AnimistFeat1", "Animist feat", 2, (ft) => ft.HasTrait(TAnimist)));
                sheet.AddAtLevel(3, values =>
                {
                    values.SetProficiency(Trait.Fortitude, Proficiency.Expert);
                });
                sheet.AddSelectionOption(new SingleFeatSelectionOption("AnimistFeat2", "Animist feat", 4, (ft) => ft.HasTrait(TAnimist)));
                sheet.AddSelectionOption(new SingleFeatSelectionOption("AnimistFeat3", "Animist feat", 6, (ft) => ft.HasTrait(TAnimist)));
                sheet.AddSelectionOption(new SingleFeatSelectionOption("AnimistFeat4", "Animist feat", 8, (ft) => ft.HasTrait(TAnimist)));
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Ally Portal"), 1, "You pull your friend alongside you.",
                "Choose a target square, then choose an adjacent ally.\n\nYou teleport as normal, then you pull your ally through the portal to an adjacent square of your choice.", [TAnimist, Trait.Flourish])
            .WithActionCost(1)
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Boomerang Portal"), 1, "You swiftly move in and out of combat.",
                "Teleport as normal, then make a single melee Strike, then teleport back to the square where you started.", [TAnimist, Trait.Flourish])
            .WithActionCost(1)
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Attack Portal"), 1, "You come out of the portal with your weapons imbued with extradimensional energy.",
                "Teleport as normal, then you gain a +2 status bonus to your next attack this turn.", [TAnimist, Trait.Flourish])
            .WithActionCost(1)
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Covering Portal"), 1, "You summon a portal in such a way that it serves as cover from attacks.",
                "You may choose to teleport as normal. Whether or not you teleport, you gain a +2 circumstance bonus to your AC until you move or until the beginning of your next turn.", [TAnimist, Trait.Flourish])
            .WithActionCost(1)
                    ...
                    return new SubmenuPossibility(IllustrationName.SteelShield, "Covering Portal")
                    {
                        ...
                        [
                            new PossibilitySection("Covering Portal")
                            {
                                Possibilities =
                                [
                                    new ActionPossibility(new CombatAction(qff.Owner, IllustrationName.SteelShield, "Shield only", [TAnimist, Trait.Flourish, Trait.Basic],
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Double-Hop Portal"), 1, "You create two portals, jump into the first one, then out the first one into the second.",
                "Teleport as normal. Then do it again.", [TAnimist, Trait.Flourish])
            .WithActionCost(1)
                ...
                qf.ProvideMainAction = qff =>
                {
                    return Wrap(CreateNormalPortal(qff.Owner, IllustrationName.EnergyEmanation, "Elemental Blast Portal", $"Create a portal as normal, and deal {S.HeightenedVariable((qff.Owner.Level + 1) / 2, 1)}d6 acid, electricity, fire, cold or sonic damage to each creature in the target square or adjacent to it (basic Reflex save against your class DC mitigates). Afterwards, you may choose to teleport there as normal.")
                        ...
                        .WithEffectOnChosenTargets((async (spell, caster, targets) =>
                        {
                            ...
                            int dc = caster.ClassDC(TAnimist);
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Shielding Portal"), 2, "You prepare to summon a portal at a moment's notice that would shunt away incoming projectiles.",
                "Until your next turn, if you'd be the target of a ranged attack (including a ranged spell attack, but not a spell that requires a save), you can spend {icon:Reaction} a reaction. If you do, the attack automatically misses as the projectile is deflected into a portal.", [TAnimist, Trait.Flourish])
            .WithActionCost(1)
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Healing Portal"), 2, "You direct your portal so that it passes through the Plane of Positive Energy.",
                "Teleport as normal, except you also heal 1d8 HP per two character levels, rounded up. You can only use Healing Portal once per encounter.", [TAnimist, Trait.Flourish, Trait.Positive, Trait.Healing])
            .WithActionCost(2)
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Stealth Portal"), 6, "You camouflage both your portal and yourself, emerging from your portal unseen.",
                "Teleport as normal, except you also become invisible as per the spell {i}invisibility.{/i}", [TAnimist, Trait.Flourish, Trait.Illusion])
            .WithActionCost(2)
...
• If it's an enemy, it makes a Reflex save against your class DC. If it fails, you swap positions. If it succeeds, the enemy stays in place but you can choose to teleport adjacent to that enemy anyway.", [TAnimist, Trait.Flourish])
            .WithActionCost(1)
            ...
            .WithPermanentQEffect("Create a bidirectional portal and pull a creature on the other hand back through.", qf =>
            {
                qf.ProvideMainAction = qff =>
                {
                    return Wrap(new CombatAction(qff.Owner, new SideBySideIllustration(IllPortal, IllustrationName.Shove), "Transposition Portal", [TAnimist, Trait.Teleportation, Trait.Move, Trait.Conjuration, Trait.Flourish, Trait.Basic], @"{i}You create a bidirectional portal and pull a creature on the other hand back through.{/i}
...
• If it's an enemy, it makes a Reflex save against your class DC. If it fails, you swap positions. If it succeeds, the enemy stays in place but you can choose to teleport adjacent to that enemy anyway.",
                        ...
                        .WithSavingThrow(new SavingThrow(Defense.Reflex, cr => cr?.ClassDC(TAnimist) ?? 10))
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Rising Portal"), 2,
                ...
                @"Stand up as {icon:FreeAction}a free action. This doesn't provoke attacks of opportunity.", [TAnimist, Trait.Flourish])
            .WithActionCost(0)
...
", [TAnimist, Trait.Flourish])
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Chained Portals"), 6,
                ...
                "Once per day, you may open up to two portals on the same turn, as though they weren't flourish actions. {i}(You do this by choosing 'Chain another portal' after opening the first portal.){/i}", [TAnimist])
            .WithPermanentQEffect("Once per day, you may open up to two portals on the same turn, as though they weren't flourish actions.", qf =>
            {
                qf.ProvideContextualAction = qff =>
                {
                    ...
                    if (portalist.Actions.ActionHistoryThisTurn.Any(action => action.HasTrait(TAnimist) && action.HasTrait(Trait.Flourish)))
                    {
                        return new ActionPossibility(new CombatAction(portalist, IllPortal, "Chain another portal", [TAnimist, Trait.Basic],
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Relentless Portal"), 8, "If your first portal strike doesn't hit, your second will. You will not leave your enemy be.",
                "Teleport as normal, then make a melee attack against an adjacent enemy. If you miss but don't critically miss, teleport again into another square adjacent to the same enemy, and make another attack. If you do, your multiple attack penalty increases twice, but only after you make the second attack.", [TAnimist, Trait.Flourish])
            .WithActionCost(1)
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Retaliatory Portal"), 8, "If your first portal strike doesn't hit, your second will. You will not leave your enemy be.",
                "After you take damage from an enemy melee attack, you can teleport to a square adjacent to that enemy as {icon:Reaction}a reaction, and make a melee Strike against that enemy.", [TAnimist])
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Apparition Sense"), 1, "You can see and interact with things others can't.", "blah blah", [TAnimist, Trait.Divine])
            .WithPermanentQEffect("Apparition Sense QEffect Text", qf =>
            {

            });
        yield return new TrueFeat(ModManager.RegisterFeatName("Channeler's Stance"), 1, "You enter a stance that allows power to flow through you.", "blah blah", [TAnimist, Trait.Stance])
            .WithActionCost(1)
            .WithPermanentQEffect("Channeler Stance QEffect Text", qf =>
            {
            });
    }
    ...
    private static CombatAction CreateNormalPortal(Creature portalist, Illustration illustration, string name, string description)
    {
        ...
        List<Trait> traits = [TAnimist, Trait.Move, Trait.Flourish, Trait.Conjuration, Trait.Teleportation];
